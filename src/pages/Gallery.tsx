...
export function Gallery() {
  ...
  useEffect(() => {
    // Small delay to ensure DOM elements exist after filter changes
    const timer = setTimeout(() => {
      const cleanup = setupScrollReveal();
      return cleanup;
    }, 50);
    return () => clearTimeout(timer);
  }, [setupScrollReveal, filter]);
            ...
            <AnimatePresence mode="popLayout">
              {filteredImages.map((img, index) => (
                <motion.div
                  ...
                  className="fade-up relative group cursor-pointer overflow-hidden rounded-2xl bg-surface-100 break-inside-avoid"
                  style={{ transitionDelay: `${((index % 6) + 1) * 0.1}s` }}
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-20 flex flex-col items-center">
            <div className="overflow-hidden pb-2 mb-6">
              <h2 className="text-4xl md:text-6xl font-heading font-black italic tracking-tighter uppercase fade-up">
                THE <span className="text-gradient-gold">TRANSFORMATION</span>
              </h2>
            </div>
            <div 
              className="w-32 h-[1px] bg-[#C9A84C] mx-auto origin-center mb-6 fade-up" 
              style={{ transitionDelay: '0.15s' }}
            />
            <p className="text-text-muted uppercase tracking-[0.4em] text-xs fade-up" style={{ transitionDelay: '0.25s' }}>Visual proof of our craft</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            {[1, 2, 3].map((i) => (
              <div key={i} className="group relative aspect-[3/4] rounded-3xl overflow-hidden glass shadow-2xl">
                {/* Before Image */}
                <img
                  src={`/images/cover.png&sig=before-${i}`}
                ...
                <div className="absolute inset-0 w-full h-full overflow-hidden transition-all duration-500 group-hover:clip-path-reveal" style={{ clipPath: 'inset(0 0 0 100%)' }}>
                  <img
                    src={`/images/cover.png&sig=after-${i}`}
